...
class VIV_files:
    ...
    def get_valid_measurements(self):
        data = [channel.data[int(self.starttimes*sampling_freq):int(self.endtimes*sampling_freq)] for channel in self.measurements.Channels]
    ...
    def spectral_density(self, name_fft, plot=False, save=True):
        ...
        timeseries = self.measurements.Channels[idx].data
        norm_timeseries = timeseries-np.mean(timeseries)
        f, Pxx = welch(norm_timeseries, fs=sampling_freq, return_onesided=True, scaling="density", nperseg=2048)
        plt.plot(f, Pxx)
        #plt.ylim(0.5e-3, 10)
        plt.xlim(0, 10)
